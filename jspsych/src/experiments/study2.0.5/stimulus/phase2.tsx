...
const Page = ({jsPsych, subcategory, sub_cat, probe, ...props}) => {
  const decorators: Decorator[] = [extractTime]
  
  // console.log(props)
  const formConfig: FormProps = {
    decorators,
    onSubmit: wrapSubmit(jsPsych),
    initialValues: {
      sub_cat,
      choice_item: jsPsych.data.get().last(2).values()[0]["participant_item"],
    }
  }
  ...
  return <Form {...formConfig}>
    {({handleSubmit, values, invalid, ...rest}) => {
      const subcat = pluralize(subcategory)
      const item = probe
